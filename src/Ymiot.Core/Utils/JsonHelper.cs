...
public static class JsonHelper
{
    ...
    public static bool LoadAssembly(string assemblyPath)
    {
        ...
        if (_jsonConverterType == null) return false;
        var jsonFormattingType = assembly.GetType("Newtonsoft.Json.Formatting");
        _serializeObjectMethodInfo =
            type.GetMethod("SerializeObject", new[] { typeof(object), _jsonConverterType.MakeArrayType() });
        _isoDateTimeConverterType = assembly.GetType("Newtonsoft.Json.Converters.IsoDateTimeConverter");

        _deserializeObjectMethodInfo = type.GetMethod("DeserializeObject",
            new[] { typeof(string), _jsonConverterType.MakeArrayType() });
    ...
    /// <param name="filePath"></param>
    /// <param name="dateTimeFormat"></param>
    public static void SaveObjectToFile<T>(T obj, string filePath, string dateTimeFormat = "yyyy-MM-dd HH:mm:ss")
        where T : class
    {
        var json = SerializeObjectToFormatJson(obj, dateTimeFormat);
